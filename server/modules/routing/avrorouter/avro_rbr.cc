...
{
public:
    EventConverter(const STableMap& map, const STableCreate& create):
        m_map(map),
        m_create(create)
    {
    }
...
protected:
    const STableMap&    m_map;
    const STableCreate& m_create;
};
...
public:
    ...
    AvroConverter(const STableMap& map, const STableCreate& create, SAvroTable table):
...
uint8_t* process_row_event_data(STableMap map, STableCreate create, EventConverter* conv,
